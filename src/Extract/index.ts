...
export class GameDataExtractor extends Transform {
  ...
  override _transform(
    ...
    const [day, month, year, hour, minute, second] = (preGame.dateTime as string)
      .split(/[.: ]/)
      .map((v, i) => (i < 3 ? +v : +v - 1));
    if (day && month && year && hour && minute && second) {
      const unixTime = new Date(year, month - 1, day, hour, minute, second).getTime() / 1000;
      callback(null, {
        unixTime,
        mapDisplayName: preGame.mapDisplayName,
        battleType: battleTypeNames.get(preGame.battleType as number),
      } as any);
      return;
    }
    callback();
    return;
    // if (!preGame) {
    //   logger.warn({ filePath }, 'Skipping file due to missing game data');
    //   callback();
    //   return;
    // }
    // if (!postGame) {
